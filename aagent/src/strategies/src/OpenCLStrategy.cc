/**
   @file    OpenCLStrategy.cc
   @ingroup OpenCLStrategy
   @brief   OpenCL search strategy
...
bool OpenCLStrategy::reqAndConfigureFirstExperiment( Region* r ) {
...
void OpenCLStrategy::configureNextExperiment() {
    Prop_List::iterator prop_it;
...
bool OpenCLStrategy::evaluateAndReqNextExperiment() {
...
std::list<Property*> OpenCLStrategy::create_initial_candidate_properties_set( Region* r ) {
    std::list<ApplProcess>           controlled_processes = dp->get_controlled_processes();
        ...
        for( reg = regions.begin(); reg != regions.end(); reg++ ) {
            ...
            for( int i = 0; i < appl->getOmpThreads(); i++ ) {
                ...
                case OPENCL_MEM_CMD_REGION:
                    //printf("OPENCL_MEM_CMD_REGION\n");
                    prop = new KernelExecutionTime( ct, pCt );
                //break;
                case OPENCL_KERNEL_CALL_REGION:
                    //printf("OPENCL_KERNEL_CALL_REGION\n");
                    prop = new KernelExecutionTime( ct, pCt );
                    break;
                default:
                    break;
...
std::list<Property*> OpenCLStrategy::create_next_candidate_properties_set( std::list<Property*> foundProperties ) {
    std::list<Property*>           candidates;
...
std::string OpenCLStrategy::name() {
    return "OpenCLStrategy";
}
