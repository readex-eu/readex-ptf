/**
   @file    StallCycleAnalysisPerSyst.cc
   @ingroup StallCycleAnalysisPerSystStrategy
   @brief   Itanium2 stall cycles PerSyst search strategy
 ...
 */
#include "global.h"
#include "StallCycleAnalysisPerSyst.h"
...
void StallCycleAnalysisPerSyst::set_propid_list_persyst( std::list<int> propID_list ) {
    psc_dbgmsg( 1, "Persyst Properties set in Strategy\n" );
    std::list<int>::iterator p;
    int                      i;
    for( i = 0; i < 50; i++ ) {
        PerSystList[ i ] = false;
    }
    for( p = propID_list.begin(); p != propID_list.end(); p++ ) {
        i                = *p;
        PerSystList[ i ] = true;
    }
    /*for(i = 0 ; i < 50 ; i++) {
       if(PerSystList[i] == true)
       psc_dbgmsg(1, "Persyst Properties set in Strategy: set %d\n", i);
       }*/
}

bool StallCycleAnalysisPerSyst::reqAndConfigureFirstExperiment( Region* r ) {
...
void StallCycleAnalysisPerSyst::configureNextExperiment() {
    Prop_List::iterator prop_it;
...
bool StallCycleAnalysisPerSyst::evaluateAndReqNextExperiment() {
    ...
    //candProperties = create_next_candidate_properties_set(foundPropertiesLastStep);   //just search Persyst Properties

    for( prop_it = candProperties.begin(); prop_it != candProperties.end();
         prop_it++ ) {
        ( *prop_it )->request_metrics();
    }
...
StallCyclesProp* StallCycleAnalysisPerSyst::duplicateAndRefine( StallCyclesProp* p, Metric m ) {
...
std::list<Property*> StallCycleAnalysisPerSyst::create_initial_candidate_properties_set( Region* r ) {
    std::list<ApplProcess>           controlled_processes = dp->get_controlled_processes();
    ...
    for( process = controlled_processes.begin();
        ...
        for( reg = regions.begin(); reg != regions.end(); reg++ ) {
            ...
            for( int i = 0; i < appl->getOmpThreads(); i++ ) {
                ...
                if( PerSystList[ 22 ] ) {
                    candidates.push_back( duplicateAndRefine( prop, PSC_BE_FLUSH_BUBBLE_BRU ) ); //test if Persyst find these properties
                }
                if( PerSystList[ 23 ] ) {
                ...
                //candidates.push_back(new StallCyclesIntegerLoadsProp(prop->get_context(), prop->get_phaseContext(), pedanticSearch ? 0.0 : IA64_PROPS_THRESHOLD::StallCyclesIntegerLoadsProp));   //searching only for Persyst properties
            }
...
std::list<Property*> StallCycleAnalysisPerSyst::create_next_candidate_properties_set( std::list<Property*> foundProperties ) {
    std::list<Property*>           candidates;
...
std::string StallCycleAnalysisPerSyst::name() {
    return "StallCycleAnalysisPerSystStrategy";
}
