...
ROMIOPlugin::ROMIOPlugin() : app( Application::instance() ) {
}
...
void ROMIOPlugin::startTuningStep( void ) {
    search_step1        = 0; //the first step is not done
    search_step2        = 0; //the second step is not done
    exist_collective_io = 0;
...
void ROMIOPlugin::createScenariosForCollectiveWriteInStep1( Region* regionPtr ) {
    ...
    tp_romio_cb_write->setRange( 1, 2, 1 );
    ...
    tp_cb_buffer_size->setRange( 2, ( int )( log( ( double )MAX_COLLECTIVE_BUFFER ) / log( ( double )2 ) ), 1 );
...
void ROMIOPlugin::createScenariosForCollectiveReadInStep1( Region* regionPtr ) {
    vector<TuningParameter*> tuningParameters;
    VariantSpace*            variantSpace;
    SearchSpace*             searchSpace;
    ...
    tp_romio_cb_read->setRange( 1, 2, 1 );
...
int ROMIOPlugin::searchForTheBestCollectiveBufferSize( Region* current_region ) {
    double best_performance = -1;
    int    best_buffer_size = -1;
    int    buffer_size      = -1;

    string       current_file_name = ( current_region->get_ident() ).file_name;
    stringstream ss;
    ss << ( current_region->get_ident() ).start_position;
    string current_line_number = ss.str();
    string current_region_key  = current_file_name + "-" + current_line_number;

    list<MetaProperty>                mp;
    std::list<MetaProperty>::iterator mpit;
    ...
    for( int scenario_id = 0; scenario_id < pool_set->fsp->size(); scenario_id++ ) {
        ...
        mp = pool_set->srp->getScenarioResultsByID( scenario_id );
        if( !mp.empty() ) {
            for( mpit = mp.begin(); mpit != mp.end(); ++mpit ) {
                PropertyPurpose purpose_for_tuning = PSC_PROPERTY_PURPOSE_TUNING;
                if( mpit->getPurpose() != purpose_for_tuning ) {
                    continue;
                }

                ss.str( "" );
                ss << mpit->getStartPosition();
                string line_number = ss.str();
                string key         = mpit->getFileName() + "-" + line_number;
                ...
                for( tsit = ts->begin(); tsit != ts->end(); ++tsit ) {
                    ...
                    for( tpit = tp.begin(); tpit != tp.end(); ++tpit ) {
                        if( !tpit->first->getName().compare( "CB_BUFFER_SIZE" ) ) {
                            buffer_size_is_set = 1;
                            buffer_size        = tpit->second;
                        }
                    } // for tpit
                }     // for tsit

                if( !buffer_size_is_set ) {
                    continue;
                }

                if( best_performance == -1 || best_performance > mpit->getSeverity() ) {
                    best_performance = mpit->getSeverity();
...
void ROMIOPlugin::createScenariosForCollectiveWriteInStep2( Region* regionPtr ) {
    int best_collective_write_buffer_size = searchForTheBestCollectiveBufferSize( regionPtr );
    ...
    tp_romio_cb_write->setRange( 2, 2, 1 );
    tp_cb_buffer_size->setRange( best_collective_write_buffer_size, best_collective_write_buffer_size, 1 );
...
void ROMIOPlugin::createScenariosForCollectiveReadInStep2( Region* regionPtr ) {
    int best_collective_read_buffer_size = searchForTheBestCollectiveBufferSize( regionPtr );
    ...
    tp_romio_cb_read->setRange( 2, 2, 1 );
    tp_cb_buffer_size->setRange( best_collective_read_buffer_size, best_collective_read_buffer_size, 1 );
    tp_cb_nodes->setRange( 0, ( int )( log( ( double )TASKS_PER_NODE ) / log( ( double )2 ) ), 1 );
...
void ROMIOPlugin::createScenarios( void ) {
    ...
    for( regionIterator = regions.begin(); regionIterator != regions.end(); regionIterator++ ) {
        if( ( *regionIterator )->get_name() == "mpi_file_write_all" ) {
            exist_collective_io = 1;
            diff_regions++;
            if( step == 1 ) {
                createScenariosForCollectiveWriteInStep1( *regionIterator );
            }
            else if( step == 2 ) {
                createScenariosForCollectiveWriteInStep2( *regionIterator );
            }
        }
            ...
            diff_regions++;
            if( step == 1 ) {
                createScenariosForCollectiveReadInStep1( *regionIterator );
            }
            else if( step == 2 ) {
                createScenariosForCollectiveReadInStep2( *regionIterator );
            }
        }
...
void ROMIOPlugin::defineExperiment( int               numprocs,
    ...
    while( !pool_set->psp->empty() ) {
        ...
        Region*        region    = Application::instance().getRegionByID( context.context_union.entity_list->front() );
        ...
        if( test_scenarios_num > 0 ) {
            for( int i = 0; i < test_scenarios_num; i++ ) {
                ...
                Region*        test_region    = Application::instance().getRegionByID( test_context.context_union.entity_list->front() );
...
bool ROMIOPlugin::searchFinished( void ) {
    ...
    if( ( exist_collective_io ) && ( !pool_set->csp->empty() || search_step1 == 0 || search_step2 == 0 ) ) {
        return false;
    }
    if( !exist_collective_io && ( !pool_set->csp->empty() || search_step1 == 0 ) ) {
        return false;
    }
                ...
                for( mpit = mp.begin(); mpit != mp.end(); ++mpit ) {
                    ...
                    if( key == current_region_key ) {
                        ...
                        if( tag[ 0 ] == 1 || tag[ 1 ] == 1 ) {
                            ...
                            printf( "%-15s|", ( mpit->getFileName() ).c_str() );
                            printf( "%-15d|", mpit->getStartPosition() );
