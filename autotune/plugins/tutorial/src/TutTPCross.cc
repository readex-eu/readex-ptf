 ...
 */
void TutTPCross::initialize( DriverContext*   context,
                             ScenarioPoolSet* pool_set ) {
    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "TutTPCross: call to initialize()\n" );

    this->context  = context;
    this->pool_set = pool_set;

    if( appl->get_regions().empty() ) {
        psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "TutTPCross plugin : No Region found. Exiting.\n" );
        throw 0;
    }
    ...
    numberOfThreadsTP->setPluginType( TUTTPCROSS );
...
//! [initialize]
    ...
    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "TutTPCross plugin: Before exiting to initialize, Tuning parameter is: %s\n\n",
                results.c_str() );
 ...
 */
//! [startTuningStep]
void TutTPCross::startTuningStep( void ) {
    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "TutTPCross: call to startTuningStep()\n" );

    std::list<Region*>           code_regions = appl->get_regions();
    ...
    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ),
                "TutTPCross: found %d parallel regions (out of %d total).\n", parallel_regions, count );
}
//! [startTuningStep]
 ...
 */
//! [analysisRequired]
bool TutTPCross::analysisRequired( StrategyRequest** strategy ) {
 ...
 */
//! [createScenarios]
void TutTPCross::createScenarios( void ) {
    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "TutTPCross: call to createScenarios()\n" );
    ...
    for( property = found_properties.begin(); property != found_properties.end(); property++ ) {
        ...
        psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ),
                    "TutTPCross: Severity (%f) in region: %s added to vector\n",
    ...
    for( int i = 0; i < noSigRegions; i++ ) {
        ...
        psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ),
                    "TutTPCross- selected region with: runtime: %f; ID: %s; file: %s; first line: %d; last line: %d;\n",
 ...
 */
//! [prepareScenarios]
void TutTPCross::prepareScenarios( void ) {
    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "TutTPCross: call to prepareScenarios()\n" );
 ...
 */
//! [defineExperiment]
void TutTPCross::defineExperiment( int               numprocs,
                                   bool&             analysisRequired,
                                   StrategyRequest** strategy ) {
    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "TutTPCross: call to defineExperiment()\n" );
 ...
 */
//! [restartRequired]
bool TutTPCross::restartRequired( std::string& env,
                                  int&         numprocs,
                                  std::string& command,
                                  bool&        is_instrumented ) {
    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "TutTPCross: call to restartRequired()\n" );
    return false;
}
//! [restartRequired]
 ...
 */
//! [searchFinished]
bool TutTPCross::searchFinished( void ) {
    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "TutTPCross: call to searchFinished()\n" );
 ...
 */
//! [finishTuningStep]
void TutTPCross::finishTuningStep( void ) {
    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "TutTPCross: call to processResults()\n" );
}
//! [finishTuningStep]
 ...
 */
//! [tuningFinished]
bool TutTPCross::tuningFinished( void ) {
    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "TutTPCross: call to tuningFinished()\n" );
    return true;
}
//! [tuningFinished]
 ...
 */
//! [getAdvice]
Advice* TutTPCross::getAdvice( void ) {
    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "TutTPCross: call to getAdvice()\n" );
 ...
 */
//! [finalize]
void TutTPCross::finalize() {
    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "TutTPCross: call to finalize()\n" );
}
//! [finalize]
 ...
 *
 */
void TutTPCross::terminate() {
    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "TutTPCross: call to terminate()\n" );
}
 ...
 * @ingroup TutTPCross
 *
 * @return A pointer to a new TutTPCross
 */
IPlugin* getPluginInstance( void ) {
    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "TutTPCross: call to getComponentInstance()\n" );

    return new TutTPCross();
}
...
int getVersionMajor( void ) {
    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "TutTPCross: call to getInterfaceVersionMajor()\n" );
...
int getVersionMinor( void ) {
    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "TutTPCross: call to getInterfaceVersionMinor()\n" );

    return 0;
}

/**
 * @brief Returns the plugin name TutTPCross.
 *
 * @ingroup TutTPCross
 *
 * @return Returns TutTPCross
 *
 */
string getName( void ) {
    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "TutTPCross: call to getName()\n" );
...
string getShortSummary( void ) {
    psc_dbgmsg( PSC_SELECTIVE_DEBUG_LEVEL( AutotunePlugins ), "TutTPCross: call to getShortSummary()\n" );

    return "cross product of tuning parameter values for multiple significant regions";
}
