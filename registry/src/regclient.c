/**
   @file    regclient.c
   @brief   Test client for the registry
...
int main( int argc, char* argv[] ) {
    ...
    for( i = 0; i < 10; i++ ) {
        ...
        len = registry_get_string_at( reg, entry_id, string_id, strbuf, 200 );
        printf( "--> read %d bytes ::%s::\n", len, strbuf );

        entry_id = registry_delete_entry( reg, entry_id );
        if( entry_id == 0 ) {
            fprintf( stderr, "Failed to delete registry entry\n" );
        }

        if( !close_registry( reg ) ) {
